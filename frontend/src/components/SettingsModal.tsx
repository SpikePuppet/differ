...
export function SettingsModal({ onClose }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setModels({
        anthropic:
          settings[1] && ANTHROPIC_MODELS.some((m) => m.value === settings[1])
            ? settings[1]
            : PROVIDER_DEFAULTS.anthropic,
        openai:
          settings[3] && OPENAI_MODELS.some((m) => m.value === settings[3])
            ? settings[3]
            : PROVIDER_DEFAULTS.openai,
        openrouter: settings[5] ?? PROVIDER_DEFAULTS.openrouter,
      });
    } catch {
